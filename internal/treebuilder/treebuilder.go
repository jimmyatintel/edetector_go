...
func treeBuilder(ctx context.Context, explorerFile string, agent string, diskInfo string) {
	...
	for _, line := range lines {
		...
		default:
			...
			err = rabbitmq.ToRabbitMQ_Details(config.Viper.GetString("ELASTIC_PREFIX")+"_explorer", &ExplorerDetails{}, nil, values, RelationMap[child].UUID, agent, ip, name, values[0], values[3], "file_table", RelationMap[child].Path, "ed_low", "StartGetDrive", taskID)
			if err != nil {
...
func treeTraversal(agent string, ind int, isRoot bool, path string, diskInfo string, UUIDMap *map[string]int, RelationMap *map[int](Relation), headData *ExplorerRelation, taskID string) {
	...
	for _, uuid := range relation.Child {
		treeTraversal(agent, (*UUIDMap)[uuid], false, path, diskInfo, UUIDMap, RelationMap, headData, taskID)
	}
}
