...
func parseScan(path string, key string) error {
	...
	for _, line := range lines {
		...
		err = rabbitmq.ToRabbitMQ_Details(config.Viper.GetString("ELASTIC_PREFIX")+"_memory", &m_tmp, nil, values, uuid, key, ip, name, values[0], values[1], "memory", values[17], "ed_mid", "StartScan", taskID)
		if err != nil {
			return err
		}
	}
...
func scanNetworkElastic(pid string, pCreateTime string, key string, data string, ip string, name string) int {
	...
	for _, line := range lines {
		...
		uuid := uuid.NewString()
		err = rabbitmq.ToRabbitMQ_Details(config.Viper.GetString("ELASTIC_PREFIX")+"_memory_network", &MemoryNetwork{}, nil, values, uuid, key, ip, name, "0", "0", "0", "0", "ed_mid", "StartScan", taskID)
		if err != nil {
