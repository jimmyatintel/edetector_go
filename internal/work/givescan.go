...
func GiveScan(p packet.Packet, conn net.Conn) (task.TaskResult, error) {
	...
	err = redis.RedisSet(key+"-ScanProgress", progress)
	if err != nil {
		return task.FAIL, err
	}
...
func GiveScanEnd(p packet.Packet, conn net.Conn) (task.TaskResult, error) {
	...
	// unzip data
	err = file.DecompressFile(srcPath, workPath, scanTotal)
	if err != nil {
		return task.FAIL, err
	}
...
func updateScanProgress(key string) {
	for {
		...
		err = query.Update_progress(scanProgress, key, "StartScan")
		if err != nil {
...
func parseScan(path string, key string) error {
	...
	for _, line := range lines {
		...
		err = rabbitmq.ToRabbitMQ_Details(config.Viper.GetString("ELASTIC_PREFIX")+"_memory", &Collect_Memory{}, &Memory{}, values, uuid, key, ip, name, values[0], values[1], "memory", values[17], "ed_mid", "StartScan", taskID, "memory")
		if err != nil {
			return err
		}
	}
...
func scanNetworkElastic(pid string, pCreateTime string, key string, data string, ip string, name string) int {
	...
	for _, line := range lines {
		...
		uuid := uuid.NewString()
		err = rabbitmq.ToRabbitMQ_Details(config.Viper.GetString("ELASTIC_PREFIX")+"_memory", &Collect_MemoryNetwork{}, &MemoryNetwork{}, values, uuid, key, ip, name, "0", "0", "0", "0", "ed_mid", "StartScan", taskID, "memory_network")
		if err != nil {
